...
const RiderLoginController = (db) => {
    ...
    router.post('/rider/login', async (req, res) => {
        ...
        try {
            ...
            db.query(
                'SELECT br.rider_id , br.name AS riderName, br.blood_type, rl.password FROM bloodbank_riders br JOIN rider_login rl ON br.rider_id = rl.rider_id WHERE rl.username = ?',
                [enteredUsername],
                async (error, results) => {
                    if (error) {
                    ...
                    bcrypt.compare(enteredPassword, storedPasswordHash, (err, passwordMatch) => {
                        ...
                        const token = jwt.sign({ userId, riderName, enteredUsername }, secretKey, {
                        ...
                        // Send the token, rider name, and blood type as part of the response
